...
function CO2() {
  ...
  const fetchCO2 = async () => {
    ...
    //!!!!!change the link here for connecting to actual backend
      .get("http://localhost:5021/Plants/airCO2")
      .then((response) => {
        setPlant(response.data);
      })
      .catch((error) => {
  ...
  const fetchNotificationData = async () => {
    try {
      const response = await axios.get("http://localhost:5021/AlertsNotification/8");
  ...
  const sendNotificationSettings = () => {
    axios
      .patch("http://localhost:5021/AlertsNotification/8", {
        id: 8,
        ...
        <div className="container">
          <div className="box1">
            <div className="lastFetched" id="left">
              ...
              <p>Last Fetched at: <TimeDisplay /> - {plant.AirCO2}%</p>
