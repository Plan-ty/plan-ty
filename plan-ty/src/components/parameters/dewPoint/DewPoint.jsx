...
function DewPoint() {
  ...
  const fetchData = async () => {
    try {
      ...
      const data = response.data.thresholds.find(item => item.type === 'dewPoint');
  ...
  const fetchNotificationData = async () => {
    try {
      const response = await axios.get("http://localhost:5021/AlertsNotification/7");
  ...
  const sendNotificationSettings = () => {
    axios
      .patch("http://localhost:5021/AlertsNotification/7", {
        id: 7,
        ...
        <div className="container">
          <div className="box1">
            <div className="lastFetched" id="left">
              ...
              <p>Last Fetched at: <TimeDisplay /> - {plant.dewPoint} °C</p>
