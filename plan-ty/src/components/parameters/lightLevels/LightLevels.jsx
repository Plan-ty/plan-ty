...
function LightLevels() {
  ...
  const fetchData = async () => {
    try {
      ...
      const data = response.data.thresholds.find(item => item.type === 'lightLevel');
  ...
  const fetchNotificationData = async () => {
    try {
      const response = await axios.get("http://localhost:5021/AlertsNotification/9");
  ...
  const sendNotificationSettings = () => {
    axios
      .patch("http://localhost:5021/AlertsNotification/9", {
        id: 9,
  ...
  return (
    <div>
      ...
      <div className="container">
        <div className="box1">
          <div className="lastFetched" id="left">
            ...
            <p>Last Fetched at: <TimeDisplay /> - {plant.lightLevel} lux</p>
      ...
      <button id="notification-button" onClick={sendNotificationSettings}>Update Notifications</button>
        ...
        <div className="graph">
          <p>Graph:</p>
          <Chart dataKey="lightLevels" yAxisLabel="Light level (lux)" />
        </div>
      </div>
    </div>  
);
