...
function VPressureDeficit() {
  ...
  const fetchVpd = async () => {
    await axios
    //!!!!!change the link here for connecting to actual backend
      .get("http://localhost:5021/Plants/vpd")
      .then((response) => {
        setPlant(response.data);
      })
      .catch((error) => {
  ...
  const fetchNotificationData = async () => {
    try {
      const response = await axios.get("http://localhost:5021/AlertsNotification/10");
  ...
  const sendNotificationSettings = () => {
    axios
      .patch("http://localhost:5021/AlertsNotification/10", {
        id: 10,
      ...
      <div className="container">
        <div className="box1">
          <div className="lastFetched" id="left">
            ...
            <p>Last Fetched at: <TimeDisplay /> - {plant.vpdLevel} kPa</p>
